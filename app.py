...
FORCED_CHANNEL = "TJUI9"
OWNER_ID = 8226014028  # ايديك
...
def is_subscribed(user_id):
    try:
        member = app.get_chat_member(FORCED_CHANNEL, user_id)
    ...
    if len(message.command) > 1 and message.command[1].startswith("ref_"):
        ...
        if referrer_id != user_id:
            ...
            if str(referrer_id) in users and str(user_id) in users:
                # زيادة العداد إذا لم يكن هذا المستخدم قد أحاله أحد
                if users[str(user_id)].get("referred_by") is None:
                    ...
                    # إذا وصل لـ 5، نمدد الاشتراك 30 يوم
        ...
        for group in groups:
            ...
            try:
                ...
                try:
                    ...
                    await app.send_message(user_id, str(e))
                # تحديث قائمة المجموعات بالـ ID الجديد
                new_groups = data.get("groups", [])
                new_groups.append(chat.id)
                new_groups.remove(group)
                update_user(user_id, {"groups": new_groups})
        await sleep(wait)
        data = get_user_data(user_id)  # تحديث البيانات في كل دورة
...
async def stats(client, message):
    ...
    users = load_users()
    total = len(users)
    active = sum(1 for u in users.values() if has_active_subscription(int(u.get("subscription_end", ""))))
