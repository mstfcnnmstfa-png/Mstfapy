# حقوق المطور: هذا البوت مبرمج من قبل youi3 (سميث)
# للتواصل: @youi3
...
# إعدادات البوت - يفضل قراءتها من متغيرات البيئة في Railway
API_ID = int(os.environ.get("API_ID", "29510141"))
API_HASH = os.environ.get("API_HASH", "14c074a5aed49dc7752a9f8d54cf4ad4")
BOT_TOKEN = os.environ.get("BOT_TOKEN", "8666985104:AAEZ_NgKD3KaaYyt1WVM4ZgQ8CMZwmZGEqE")
...
async def login(_: Client, callback: CallbackQuery):
    ...
    reMarkup = Markup([[Button("- العودة -", callback_data="account")]])
    try:
        ask = await wait_for_message(user_id, "- أرسل رقم هاتفك:\n\n- تقدر ترسل /cancel لإلغاء العملية.", ForceReply(selective=True, placeholder="+9647700000"), 30)
    except asyncio.TimeoutError:
        return await callback.message.reply("- انتهى وقت استلام رقم الهاتف.", reply_markup=reMarkup)
    if ask.text == "/cancel":
...
async def registration(message: Message):
    ...
    reMarkup = Markup([[Button("- إعادة المحاولة -", callback_data="login"), Button("- العودة -", callback_data="account")]])
    ...
    try:
        code = await wait_for_message(user_id, "- تم إرسال الكود إلى حسابك، أرسله هنا.", ForceReply(selective=True, placeholder="1 2 3 4 5 6"), 120)
    except asyncio.TimeoutError:
    ...
    try:
        ...
        try:
            password = await wait_for_message(user_id, "- أدخل كلمة مرور التحقق بخطوتين.", ForceReply(selective=True, placeholder="- كلمة المرور: "), 180)
        except asyncio.TimeoutError:
    ...
    write(users_db, users)
    await app.send_message(user_id, "- تم تسجيل الدخول بحسابك، الآن تقدر تستمتع بميزات البوت.",
...
async def newSuper(_: Client, callback: CallbackQuery):
    ...
    if user_id != owner and not users[str(user_id)]["vip"]:
        return await callback.answer("- انتهت مدة اشتراكك.", show_alert=True)
    await callback.message.delete()
    reMarkup = Markup([[Button("- حاول مرة ثانية -", callback_data="newSuper"), Button("- العودة -", callback_data="toHome")]])
    try:
        ask = await wait_for_message(user_id,
                                     ...
                                     ForceReply(selective=True, placeholder="- رابط السوبر: "), 60)
    except asyncio.TimeoutError:
    ...
    write(users_db, users)
    await ask.reply("- تم إضافة السوبر للقائمة.", reply_markup=Markup([[Button("- الصفحة الرئيسية -", callback_data="toHome")]]),
...
async def currentSupers(_: Client, callback: CallbackQuery):
    ...
    for group in groups:
        try:
            ...
            continue
    markup = [[Button(str(group) if titles.get(str(group)) is None else titles[str(group)], callback_data=str(group)),
               Button("🗑", callback_data=f"delSuper {group}")] for group in groups]
...
async def newCaption(_: Client, callback: CallbackQuery):
    ...
    if user_id != owner and not users[str(user_id)]["vip"]:
        return await callback.answer("- انتهت مدة اشتراكك.", show_alert=True)
    await callback.message.delete()
    reMarkup = Markup([[Button("- حاول مرة ثانية -", callback_data="newCaption"), Button("- العودة -", callback_data="toHome")]])
    try:
        ask = await wait_for_message(user_id, "- أرسل الكليشة الجديدة.\n\n- استخدم /cancel لإلغاء العملية.",
                                     ForceReply(selective=True, placeholder="- الكليشة الجديدة: "), 120)
    except asyncio.TimeoutError:
    ...
    write(users_db, users)
    await ask.reply("- تم تعيين الكليشة الجديدة.", reply_to_message_id=ask.id,
                    reply_markup=Markup([[Button("- الصفحة الرئيسية -", callback_data="toHome")]]))
...
async def waitTime(_: Client, callback: CallbackQuery):
    ...
    if user_id != owner and not users[str(user_id)]["vip"]:
        return await callback.answer("- انتهت مدة اشتراكك.", show_alert=True)
    await callback.message.delete()
    reMarkup = Markup([[Button("- حاول مرة ثانية -", callback_data="waitTime"), Button("- العودة -", callback_data="toHome")]])
    try:
        ask = await wait_for_message(user_id, "- أرسل مدة الانتظار بين كل رسالة وأخرى (بالثواني).\n\n- استخدم /cancel لإلغاء العملية.",
                                     ForceReply(selective=True, placeholder="- المدة: "), 120)
    except asyncio.TimeoutError:
    ...
    write(users_db, users)
    await ask.reply("- تم تعيين مدة الانتظار.", reply_to_message_id=ask.id,
                    reply_markup=Markup([[Button("- الصفحة الرئيسية -", callback_data="toHome")]]))
...
async def startPosting(_: Client, callback: CallbackQuery):
    ...
    elif not users[str(user_id)].get("groups"):
        return await callback.answer("- ماكو سوبرات مضافة.", show_alert=True)
    ...
    markup = Markup([[Button("- إيقاف النشر -", callback_data="stopPosting"), Button("- عودة -", callback_data="toHome")]])
...
async def stopPosting(_: Client, callback: CallbackQuery):
    ...
    write(users_db, users)
    markup = Markup([[Button("- بدء النشر -", callback_data="startPosting"), Button("- عودة -", callback_data="toHome")]])
...
async def posting(user_id):
    if users[str(user_id)]["posting"]:
        client = Client(str(user_id), api_id=app.api_id, api_hash=app.api_hash,
                        session_string=users[str(user_id)]["session"])
        await client.start()
    while users[str(user_id)]["posting"]:
        sleepTime = users[str(user_id)].get("waitTime", 60)
        groups = users[str(user_id)]["groups"]
        if "caption" not in users[str(user_id)]:
            users[str(user_id)]["posting"] = False
            write(users_db, users)
            await app.send_message(int(user_id), "- تم إيقاف النشر بسبب ماكو كليشة.",
...
async def safety_instructions(_: Client, callback: CallbackQuery):
    text = ("🔒 **تعليمات الأمان** 🔒\n\n"
            ...
            "4. إذا واجهت أي مشكلة، تواصل مع المطور @youi3.\n\n"
...
adminMarkup = Markup([
    [Button("- إلغاء VIP -", callback_data="cancelVIP"), Button("- تفعيل VIP -", callback_data="addVIP")],
...
async def addVIP(_: Client, callback: CallbackQuery):
    ...
    await callback.message.delete()
    try:
        ask = await wait_for_message(user_id, "- أرسل ايدي المستخدم لتفعيل VIP له",
                                     ForceReply(selective=True, placeholder="- user id: "), 30)
    except asyncio.TimeoutError:
    ...
    try:
        limit = await wait_for_message(user_id, "- أرسل عدد الأيام للاشتراك.\n\n- ارسل /cancel للإلغاء.",
                                       ForceReply(selective=True, placeholder="- عدد الأيام: "), 30)
    except asyncio.TimeoutError:
    ...
    try:
        await app.send_message(_id, "- تم تفعيل VIP لك في بوت النشر التلقائي" + caption.split("جديد", 1)[1])
    except:
...
async def cancelVIP(_: Client, callback: CallbackQuery):
    ...
    await callback.message.delete()
    try:
        ask = await wait_for_message(user_id, "- أرسل ايدي المستخدم لإلغاء VIP الخاص به",
                                     ForceReply(selective=True, placeholder="- user id: "), 30)
    except asyncio.TimeoutError:
        return await callback.message.reply("- انتهى وقت استلام الايدي.", reply_markup=reMarkup)
...
async def channelsControl(_: Client, callback: CallbackQuery):
    ...
    markup.extend([[Button("- إضافة قناة جديدة -", callback_data="addChannel")], [Button("- الصفحة الرئيسية -", callback_data="toAdmin")]])
...
async def addChannel(_: Client, callback: CallbackQuery):
    user_id = callback.from_user.id
    reMarkup = Markup([[Button("- العودة للقنوات -", callback_data="channels")]])
    await callback.message.delete()
    try:
        ask = await wait_for_message(user_id, "- أرسل معرف القناة بدون @.",
                                     ForceReply(selective=True, placeholder="- channel username: "), 30)
    except asyncio.TimeoutError:
...
async def vipCanceler(user_id):
    ...
    while True:
        ...
        if current.strftime("%Y-%m-%d %H:%M") >= users[str(user_id)]["limitation"]["endDate"] + " " + users[str(user_id)]["limitation"]["endTime"]:
            break
        await sleep(20)
    users[str(user_id)] = {"vip": False}
...
# ---------- HELPER FUNCTIONS ----------
async def wait_for_message(user_id, text, reply_markup, timeout):
    """إرسال رسالة وانتظار رد المستخدم خلال timeout"""
    sent = await app.send_message(user_id, text, reply_markup=reply_markup)
    response = await app.wait_for_message(filters.user(user_id), timeout=timeout)
    await sent.delete()  # حذف رسالة الطلب بعد استلام الرد
    return response
